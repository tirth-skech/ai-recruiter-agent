...
from database import get_audit_logs, init_db, log_audit
...
# --- 2. LIVE GEMINI SEARCH GROUNDING & LINK ROUTING ---
def fetch_live_gemini_jobs(search_term="AI Engineer"):
    """Fetches real-time, active job listings using Gemini 2.5 Flash with Google Search Grounding.

    Returns a maximum of 3 active, high-quality job recommendations.
    """
    ...
    - "job_id": a unique string string ID
    - "title": Job title
...
with active_tabs[1]:
    ...
    if fetch_clicked:
        with st.spinner(
            f"Searching live web for open '{search_keyword}' roles via Gemini"
            " Grounding..."
        ):
            raw_posts = fetch_live_gemini_jobs(search_keyword)
            ...
            log_audit(
                ...
                "FETCH_LIVE_JOBS",
                f"Queried live web via Gemini Grounding for role: {search_keyword}",
            ...
            with st.expander(
                ...
                with c1:
                    ...
                    st.link_button(
                        ...
                        post.get("job_url", "[https://google.com](https://google.com)"),
...
with active_tabs[2]:
    ...
    graph.node(
        "B",
        f"🌐 Gemini Live Search\nGrounding: '{target_role}'",
        shape="box",
        style="filled",
        fillcolor="#FFF3E0",
    )
...
with active_tabs[3]:
    ...
    if auth["role"] in ["Admin", "Manager"]:
        ...
        st.info(
            "Please log in as internal staff to view system audit logs."
        )
