...
# --- 3. MAIN DASHBOARD ---
    ...
    # Initialize session state for processing to prevent button "flicker"
    ...
    # --- TAB 2: PIPELINE & COLLABORATION ---
    with tab_pipe:
        st.header("Candidate Lifecycle & Team Feedback")
        try:
            ...
            if not df.empty:
                ...
                with col_note:
                    ...
                    selected_cand = st.selectbox("Review Candidate", df['candidate_name'].unique())
                    note = st.text_area(f"Notes for {selected_cand}")
                    if st.button("Save Feedback"):
                        # In Week 6, this would save to team_notes table
                        st.toast(f"Feedback synced for {selected_cand}!")
                        st.balloons()
            else:
                st.info("No candidates processed yet.")
        except Exception as e:
            st.info("Awaiting first processing run to initialize database views.")
    ...
    if auth["role"] == "Admin":
        with tab_stats:
            ...
            if 'df' in locals() and not df.empty:
                ...
                with c2:
                    st.plotly_chart(px.box(df, x='education_tier', y='expected_salary', title="Salary Range by Tier"), use_container_width=True)
                ...
                try:
                    ...
                    st.caption("No API logs recorded yet.")
            else:
