...
def init_db():
    ...
    # Cache Table for Google Search API Results
    ...
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_google_cache ON"
        " google_search_cache(search_keyword)"
    )
    conn.commit()
    return conn
...
def save_candidate_v8(conn, data, job_id, prediction):
    ...
    if exists:
        ...
        cursor.execute(
            ...
            (
                ...
                email,
                data.get("job_role", "Applicant"),
                data.get("edu_tier", "Tier-2"),
                data.get("gender"),
                data.get("ethnicity"),
                skills_str,
                gaps_str,
                recs_str,
                data.get("salary_exp", 0.0),
                data.get("score", 0),
                data.get("match_score", 0.0),
                data.get("projected_score", 0.0),
                prediction,
    ...
    log_audit(
        ...
        f"{data.get('name')} profile analyzed against"
        f" {data.get('job_role', 'Job Role')}",
        f"CAND-{str(data.get('name')).upper().replace(' ', '-')}",
    )


def log_audit(
    conn, component, action, description, assoc_data="N/A", ip="127.0.0.1"
):
    cursor = conn.cursor()
    cursor.execute(
...
def get_audit_logs(conn):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT timestamp, component, action, description, associated_data,"
        " ip_address FROM audit_logs ORDER BY id DESC"
    )
...
def cache_search_results(conn, keyword, payload_json):
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO google_search_cache (search_keyword, payload, timestamp)"
        " VALUES (?, ?, ?)",
...
def get_cached_search_results(conn, keyword):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT payload FROM google_search_cache WHERE search_keyword = ?"
