...
def init_db():
    """Initializes the relational database schema. No external project imports allowed."""
    ...
    # 2. Recruitment Pipeline (Relational Table)
...
def save_full_lifecycle(conn, data, email, latency=0, steps=None, overrides=None):
    """Saves candidate data. Logic is self-contained to avoid circularity."""
    cursor = conn.cursor()
    try:
        ...
        # Fetch the ID of the candidate we just handled
        cursor.execute("SELECT id FROM candidates WHERE name=?", (data.get('name'),))
        c_id = cursor.fetchone()[0]

        # Handle Human-in-the-Loop Overrides
